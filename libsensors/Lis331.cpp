      ...
      mInputReader(32)
{
    ...
    if (!ioctl(dev_fd, LIS331DLH_IOCTL_SET_ENABLE, &flags)) {
        if (flags)  {
...
int LisSensor::enable(int32_t handle, int en)
{
    ...
    if ((uint32_t(newState)<<what) != (mEnabled & (1<<what))) {
        ...
        int cmd;
        switch (what) {
            case Accelerometer: cmd = LIS331DLH_IOCTL_SET_ENABLE;  break;
        }
...
void LisSensor::processEvent(int code, int value)
{
    switch (code) {
        ...
        case EVENT_TYPE_ACCEL_Y:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.z = value * CONVERT_A_Y;
            break;
        case EVENT_TYPE_ACCEL_Z:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.y = value * CONVERT_A_Z;
            break;
    }
}
