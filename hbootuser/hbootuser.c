...
int select_buffer(int buffer) {
	int ret;
	printf("select_buffer - before ioctl, buff %d\n", buffer);
    ret = ioctl(ctrlfd, HBOOT_SELECT_BUFFER, buffer);
    printf("select_buffer - after ioctl\n");
	if (ret < 0) {
	    printf("select_buffer - ret < 0\n");
		perror("ioctl select_buffer");
	}
	return ret;
}
...
int handle_file(FILE *fp, int tag, int *buf) {
	...
	printf("handle file, before allocate_buffer: tag %d, filesize %d, btype %d, battrs %d\n", tag, filesize, btype, battrs);
	if ((*buf = allocate_buffer(tag, filesize, btype, battrs, 0)) == INVALID_BUFFER_HANDLE) {
		printf("failed to allocate buffer\n");
		close(fd);
		return -1;
	}
	printf("handle file - before select_buffer, buff %d\n", *buf);
	select_buffer(*buf);
